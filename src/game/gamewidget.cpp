...
void GameWidget::gameStart()
{
    ...
    // 2. 모든 리스트 비우기 (이전 게임 잔여물 제거)
    m_projectiles.clear();
    m_enemyMissiles.clear();
    m_explosions.clear();
    m_enemies.clear();
    // (아이템 기능이 있다면 m_items.clear(); m_floatingTexts.clear(); 도 추가)
    ...
    // 5. 시간 관련 초기화
    m_gameClock.restart(); // 시계 0부터 다시 시작
    m_nextEnemySpawnTime = m_gameClock.elapsed() + 2000; // 2초 후 첫 스폰
    ...
    // 7. 포커스 설정 (키보드 입력 등을 위해)
    //this->setFocus();
}

void GameWidget::gameFinished()
{
    // 1. [핵심] 게임 루프 정지
    m_gameTimer->stop();
    ...
    // 2. 객체 정리 (요청하신 대로 비우기)
    // 화면을 깔끔하게 비우고 싶다면 수행.
    // (보통은 게임 오버 장면을 배경에 남겨두기 위해 비우지 않기도 합니다. 취향껏 선택하세요.)
    m_projectiles.clear();
    m_enemyMissiles.clear();
    m_explosions.clear();
    m_enemies.clear();
    ...
    // 3. 디버그 로그
    qDebug() << "Game Logic Stopped. Final Score:" << m_score;

    // 4. [핵심] 메인 윈도우에게 알림 (점수 전달)
    emit sigGameFinished(m_score);
}
